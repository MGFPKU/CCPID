...
35. Do not round-trip exported workbooks (`outputs/` or `docs/` xlsx) through openpyxl load/save: openpyxl drops the x14 conditional-formatting extension, which strips the solid-fill data bars from the 概览/Summary sheet (the `Conditional Formatting extension is not supported and will be removed` warning signals this). For surgical edits on exported workbooks, patch `inputs/template_*.xlsx` and re-export, or edit the xlsx XML at zip level. If data bars are lost, restore them with `_apply_solid_databars` from `scripts/export_workbooks.py`.
36. Summary-sheet layout edits (概览/Summary table positions, column groups, data-bar ranges) must be captured in `inputs/template_*.xlsx` AND in `SUMMARY_LAYOUT` / `SUMMARY_DATABAR_RANGES` / the width groups in `scripts/export_workbooks.py`. The Summary sheet's static labels, formulas, merges, widths, and sheet view live in the template; the script re-writes formulas and data bars at the layout addresses, so both must move together. Similarly, header-structure edits on 工具总览/Instruments overview (e.g. deleting the title row) must update the data start row and delete guard in `write_instruments_overview_sheet` plus the B5 COUNTA offset in `write_summary_sheet`. 路径汇总/Approaches does not exist in the templates at all — the sheet, its headers, and its header styling are fully generated by `write_approaches_sheet`, so edits there touch the script only; its data-row autofit heights live in `APPROACHES_DATA_ROW_HEIGHTS` (position list) and the overview's in `OVERVIEW_ROW_HEIGHTS` (keyed by instrument ID). Data-sheet header edits (rows 1-2: group labels, header styles, merges, row heights) are template-static — capture by copying rows 1-2 values/styles/heights plus row-1 merges from the edited docs into `inputs/template_*.xlsx` with openpyxl; write cells before re-merging (MergedCell is read-only). Row-2 header values are the script's column-name mapping, so they must stay in sync with the CSV headers.
37. When a standard is superseded by a newer version (e.g. GB 32047-2015 replaced by GB 32047-2025), keep ONE row for the current version and delete the old-version row from the category CSV — do not move it to the ended CSV. The kept row keeps the earliest predecessor's adoption/start dates, uses the current version's date as `Last revision`, and mentions the replaced standard version in `Description` and `Last revision (Details)`.
...
6. Fill the draft core fields:
   `Policy Instrument ID`, `Instrument / subscheme`, `Group`, `Approach`, one policy name, `Jurisdiction level`, and `Status`.
7. Before committing the ID, check that its group, approach, and `S000`/subscheme suffix match the row classification and boundary.
