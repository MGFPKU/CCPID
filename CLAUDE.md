...
- **作用渠道 (Functioning channel)**: single-select only. `供给侧`/`需求侧`/`环境` (Supply-side/demand-side/environment). 供给侧 = directly affects producers (firms, farms, generators); 需求侧 = directly affects consumers (households, car owners, drivers); 环境 = establishes enabling conditions (evaluations, financial frameworks, voluntary standards, education). Match the 受规制主体: firms → 供给侧, households → 需求侧, framework/enabler instruments → 环境.
